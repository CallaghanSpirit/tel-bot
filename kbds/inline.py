...
def get_user_cart(
    *,
    ...
    if page:
        ...
        keyboard.row(*row)
        row2 = [
            InlineKeyboardButton(text='На главную', callback_data=MenuCallBack(level=level, menu_name='main').pack()),
            InlineKeyboardButton(text='Заказать', callback_data=MenuCallBack(level=level, menu_name='order').pack()),
        ]
        return keyboard.row(*row2).as_markup()
    else:
        keyboard.add(InlineKeyboardButton(text='На главную', callback_data=MenuCallBack(level=level, menu_name='main').pack()))
